...
export default function CaseStudyHero({ study }: { study: CaseStudy }) {
  ...
  return (
    <section id="about-project" aria-labelledby="case-study-title" className="border-t border-rule pt-8 lg:pt-[clamp(32px,4vw,56px)]">
      ...
      <p className="mt-6 max-w-prose-lg text-lead text-body">{full ? study.intro : study.description}</p>

      {full && (
        <div className="mt-8 lg:mt-[clamp(32px,3.5vw,48px)] grid grid-cols-1 sm:grid-cols-2 gap-grid-gap">
          <div className="flex flex-col gap-2 bg-panel rounded-card p-card-pad">
            ...
            <p className="m-0 text-[clamp(15px,1.2vw,17px)] leading-snug font-bold text-ink">{study.roleLabel}</p>
          </div>
          <div className="flex flex-col gap-2 bg-panel rounded-card p-card-pad">
            <p className="font-mono-label text-mono-label uppercase text-muted m-0">Focus</p>
            <p className="m-0 text-[clamp(15px,1.2vw,17px)] leading-snug font-bold text-ink">{study.focusLabel}</p>
          </div>
        </div>
      )}
    </section>
